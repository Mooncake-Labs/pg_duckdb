...
public:
	MooncakeMultiFileReader(MooncakeTable &_table) : table(_table), metadata(nullptr) {
	}
	...
	Create(const TableFunction &table_function) {
		...
		return make_uniq<MooncakeMultiFileReader>(function_info.table);
	}

	shared_ptr<MultiFileList>
	CreateFileList(ClientContext &context, const vector<string> &, FileGlobOptions) override {
	...
	// DynamicFilterPushdown(ClientContext &, const MultiFileList &, const MultiFileOptions &,
	...
	Bind(MultiFileOptions &, MultiFileList &, vector<LogicalType> &return_types, vector<string> &names,
		...
		// TODO: bind_data.mapping = MultiFileColumnMappingMode::BY_FIELD_ID;
		// bind_data.file_row_number_idx = names.size();
		return true;
	}

	// unique_ptr<MultiFileReaderGlobalState>
	// InitializeGlobalState(ClientContext &context, const MultiFileOptions &, const MultiFileReaderBindData &,
	//                       const MultiFileList &, const vector<MultiFileReaderColumnDefinition> &,
	//                       const vector<ColumnIndex> &) override {
	// 	LazyInitialize(context);
	...
	//                     const vector<MultiFileReaderColumnDefinition> &, const vector<ColumnIndex>
	//                     &global_column_ids, MultiFileReaderData &reader_data, const MultiFileReaderBindData
	//                     &bind_data, const string &, optional_ptr<MultiFileReaderGlobalState>) override {
	...
	// FinalizeChunk(ClientContext &context, const MultiFileBindData &bind_data,
	//                                       BaseFileReader &reader, const MultiFileReaderData &reader_data,
	//                                       DataChunk &input_chunk, DataChunk &output_chunk, ExpressionExecutor
	//                                       &executor, optional_ptr<MultiFileReaderGlobalState> global_state)
	// void
	// FinalizeChunk(ClientContext &context, const MultiFileBindData &bind_data,
	//               BaseFileReader &reader, const MultiFileReaderData &reader_data, DataChunk &chunk,
...
private:
	...
	LazyInitialize(ClientContext &context) {
		metadata = &table.GetTableMetadata(context);
...
MooncakeTable::GetScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data) {
	TableFunction mooncake_scan =
	    ExtensionUtil::GetTableFunction(*context.db, "parquet_scan").functions.GetFunctionByOffset(0);
