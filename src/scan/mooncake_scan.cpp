...
struct MooncakeFunctionInfo : public TableFunctionInfo {
	MooncakeFunctionInfo(TableCatalogEntry &_table) : table(_table) {
	}
...
public:
	...
	void
	LazyInitialize(MooncakeTableMetadata::DataFiles _data_files) {
		data_files = std::move(_data_files);
	}
	...
	GetTotalFileCount() override {
		return data_files.len;
	}

protected:
	string
	GetFile(idx_t i) override {
		return i < data_files.len ? data_files.Get(i) : "";
	}

private:
	MooncakeTableMetadata::DataFiles data_files;
};

class MooncakeMultiFileReader : public MultiFileReader {
public:
	MooncakeMultiFileReader(TableCatalogEntry &_table) : table(_table) {
	}
...
private:
	...
	void
	LazyInitialize() {
		auto metadata = GetMooncakeTableMetadata();
		file_list->LazyInitialize(std::move(metadata.data_files));
		for (idx_t i = 0; i < metadata.position_deletes.len; i++) {
			auto position_delete = metadata.position_deletes.Get(i);
...
TableFunction
GetMooncakeScanFunction(ClientContext &context, TableCatalogEntry &table, idx_t cardinality,
	...
	mooncake_scan.function_info = make_shared_ptr<MooncakeFunctionInfo>(table);
	vector<Value> inputs {""};
